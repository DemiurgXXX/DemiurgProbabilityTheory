...
# (100 + 80 + 75 + 77 + 89 + 33 + 45 + 25 + 65 + 17 + 30 + 24 + 57 + 55 + 70 + 75 + 65 + 84 + 90 + 150) / 20 = 65.3
...
# смещенная оценка дисперсии  = сумма (каждый элемент - среднее) ** 2 / кол-во элементов =   19002.2 / 20 = 950.11

smesh_sigma2 = temp_sum / list_smesh_len
print(f'Смещённая оценка дисперсии: {smesh_sigma2}')

# несмещенная оценка дисперсии
list_nesmesh_len = len(list) - 1    # то для получения несмещённой дисперсии отнимаем от кол-ва элементов 1  :  20 - 1 = 19
# 19002.2 / 19 = 1000.1157894736842
nesmesh_sigma2 = temp_sum / list_nesmesh_len
print(f'Несмещённая оценка дисперсии: {nesmesh_sigma2}')

# среднее квадратичное отклонение  - корень из сигмы
...
print (f'Вероятность из первого ящика из 3 чёрных вытянуть 2, из второго ящика из 5 белых вытащить 3 и из 7 чёрных вытащить 1 равна {round(p1 * 100, 3)} %')
...
print (f'Вероятность из первого ящика из 3 чёрных вытянуть 1 и из 5 белых вытянуть 1, из второго ящика из 5 белых вытащить 2 и из 7 чёрных вытащить 2 {round(p2 * 100, 3)} %')
...
print (f'Вероятность из первого ящика из 5 белых вытянуть 2 , из второго ящика из 5 белых вытащить 1 и из 7 чёрных вытащить 3 равна {round(p3 * 100, 3)} %')

# т.к нас устранивает любой из исходов то между ними ставим "или", а это значит что доли суммируем
...
Pb1 = Pb2 = Pb3 = 1/3   # Вероятность того что стрелок выстрелил у всех трёх стрелков одинакова и равна 1 / 3
...
print(f'Полная вероятность = 1/3 * 0,9 + 1/3 * 0,8 + 1/3 * 0,6 = {Pa} ')

Pb1a = (Pb1 * Pab1) / Pa
Pb2a = (Pb2 * Pab2) / Pa
Pb3a = (Pb3 * Pab3) / Pa

print(f'Вероятность того, что выстрел произведен первым спортсменом: 1/3 * 0,9 / 0.766 * 100 % =  {round((Pb1a) * 100, 3)} %')
print(f'Вероятность того, что выстрел произведен вторым спортсменом: 1/3 * 0,8 / 0.766 * 100 % = {round((Pb2a) * 100, 3)} %')
print(f'Вероятность того, что выстрел произведен третим спортсменом: 1/3 * 0,6 / 0.766 * 100 % = {round((Pb3a) * 100, 3)} %')
...
print(f'Полная вероятность сдать первую сессию: 1/4 * 0,8 + 1/4 * 0,7 + 1/2 * 0,9 = {Pa} ')

Pb1a = (Pb1 * Pab1) / Pa
Pb2a = (Pb2 * Pab2) / Pa
Pb3a = (Pb3 * Pab3) / Pa

print(f'Вероятность того, что студент учиться на факультете А: 1/4 * 0,8 / 0.825 * 100% = {round((Pb1a) * 100, 3)} %')
print(f'Вероятность того, что студент учиться на факультете В: 1/4 * 0,7 / 0.825 * 100% = {round((Pb2a) * 100, 3)} %')
print(f'Вероятность того, что студент учиться на факультете С: 1/2 * 0,9 / 0.825 * 100% = {round((Pb3a) * 100, 3)} %')
...
print(f'Вероятность выхода из строя всех деталей: (0.1 * 0.2 * 0.25) * 100 % =  {round((p1 * p2 * p3) * 100, 3)} %')

# б). только две детали
# могут выйти из строя                              1 и 2 раб, 3 слом   или 1 и 3 раб, 2 - слом или  2 и 3 раб, 1 слом
print(f'Вероятность выхода из строя 2 деталей: (0.1*0.2*0.75 + 0.1*0.8*0.25 + 0.9*0.2*0.25) *100 % = {round(( (p1 * p2 * q3) + (p1 * p3 * q2) + (p2 * p3 * q1)) * 100, 3)} %')

# в). хотя бы одна деталь
#  Имеет место обратная ситуация тому, что                       все работают
print(f'Вероятность выхода из строя хотя бы 1 детали: (1 - 0.9*0.8*0.75) * 100 % = {round((1 - q1 * q2 * q3) * 100, 3)} %')

# г). от одной до двух деталей
#  Имеет место обратная ситуация тому, что                              все сломаны   или все работают
print(f'Вероятность выхода из строя от одной до двух деталей: (1 - 0,1*0,2*0,25 - 0.9*0.8*0.75) * 100% =  {round((1 - (p1 * p2 * p3) - (q1 * q2 * q3)) * 100, 3)} %')